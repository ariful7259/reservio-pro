...
interface RentalCategoryFilterFormProps {
  category: any;
...
const RentalCategoryFilterForm: React.FC<RentalCategoryFilterFormProps> = ({
  category,
  selectedSubcategory,
  selectedLocation,
  priceRange,
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{category.count}{language === 'bn' ? 'টি সামগ্রী' : ' items'}</p>
          </div>
        </div>
            ...
            <Select value={selectedSubcategory} onValueChange={onSubcategoryChange}>
              ...
              <SelectContent>
                ...
                {category?.subcategories?.map((sub: any, index: number) => (
                  <SelectItem key={index} value={sub.id || sub.name}>
                    {sub.name || sub}
                  </SelectItem>
                ))}
              </SelectContent>
        ...
        {/* Rental Types */}
        {category.rentalTypes && (
          <div className="mt-4">
            <h4 className="font-medium mb-2">
              {language === 'bn' ? 'ভাড়ার ধরন:' : 'Rental Types:'}
            </h4>
            <div className="flex flex-wrap gap-2">
              {category.rentalTypes.map((type: string, index: number) => (
...
export default RentalCategoryFilterForm;
