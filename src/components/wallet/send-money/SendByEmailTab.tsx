...
interface SendByEmailTabProps {
  currentBalance: number;
  onSuccess: () => void;
  onClose: () => void;
}

export const SendByEmailTab: React.FC<SendByEmailTabProps> = ({
  currentBalance,
  onSuccess,
  onClose
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: recipientProfile, error: recipientError } = await supabase
        ...
        .eq('email', formData.recipientEmail.toLowerCase())
        .single();

      if (recipientError || !recipientProfile) {
        toast({
          title: 'ত্রুটি',
          description: 'প্রাপক খুঁজে পাওয়া যায়নি। ইমেইল যাচাই করুন',
          variant: 'destructive'
        });
        return;
      }
      ...
      const { error: senderTxError } = await supabase
        ...
        .insert({
          ...
          status: 'completed',
          metadata: { recipient_email: formData.recipientEmail, recipient_name: recipientProfile.full_name }
        });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4">
      <div>
        <Label htmlFor="recipientEmail">প্রাপকের ইমেইল</Label>
        <Input
          id="recipientEmail"
          type="email"
      ...
      <div>
        <Label htmlFor="emailAmount">পরিমাণ (৳)</Label>
        <Input
          id="emailAmount"
          type="number"
          step="0.01"
      ...
      <div>
        <Label htmlFor="emailDescription">বিবরণ (ঐচ্ছিক)</Label>
        <Textarea
          id="emailDescription"
